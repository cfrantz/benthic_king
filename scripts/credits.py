...
def hack(project, config):
    ...
    project.rom.free(CREDITS_DATA, CREDITS_LEN)
    ...
    for (i, item) in enumerate(CREDITS):
        ...
        val.extend(text)
        val.append(0xFF)
        dest = project.rom.alloc(CREDITS_DATA, len(val))
        project.rom.write_bytes(dest, bytes(val))
        project.rom.write_pointer(CREDITS_POINTERS + i*4, dest)
        ...
        dest = project.rom.alloc(CREDITS_DATA, len(val))
        project.rom.write_bytes(dest, bytes(val))
        project.rom.write_pointer(CREDITS_POINTERS + i*4 + 2, dest)
